...
//    }
    ...
    public Asset getAssetById(Integer assetId) {
        return assetDao.selectByPrimaryKey(assetId); // 使用Mapper提供的方法根据ID获取模型
    }

    public void insertAsset(Asset asset) {
        assetDao.insertSelective(asset); // 使用Mapper提供的方法插入模型数据
    }

    public void deleteAssetById(Integer assetId) {
        assetDao.deleteByAssetId(assetId); // 使用Mapper提供的方法根据ID删除模型
    }
